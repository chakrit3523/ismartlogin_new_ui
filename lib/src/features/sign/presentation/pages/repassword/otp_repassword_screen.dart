...
import 'package:ismart_login/src/features/sign/presentation/pages/repassword/repassword_screen.dart';
...
// API
  ...
  Future<void> _requestOtpAgain() async {
    ...
    try {
      final result =
          await MemberFuture().apiPostOtp({"PHONE": _items['PHONE']});
    ...
    try {
      await new MemberFuture().apiGetCheckOtp(map).then((onValue) {
        ...
        if (_resultOtp.isEmpty) {
          loadingDialog.dismiss();
          DialogHelper.showError(
              context, 'เกิดข้อผิดพลาด', 'ไม่พบผลการตรวจสอบ OTP');
          return;
        }
